...
`npm run verify`는 데이터 테스트, strict 데이터 검사, Cloudflare Pages 빌드, Playwright 브라우저 검사를 순서대로 실행합니다. 브라우저 검사는 데스크톱과 `390x844` 모바일에서 초기 카드, 정확 검색 순위, 페이지네이션, 키보드 포커스, 실제 콘텐츠 폭 기준 가로 넘침, 첫 화면 높이, 업데이트 내역 포커스 여백과 고정 패널 경계를 확인합니다.
